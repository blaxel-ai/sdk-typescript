...
class BlAgent {
  ...
  get url() {
    ...
    if (process.env[`BL_AGENT_${envVar}_SERVICE_NAME`]) {
      return new URL(`https://${process.env[`BL_AGENT_${envVar}_SERVICE_NAME`]}.${settings.runInternalHostname}`);
    }
    return this.externalUrl
  }

  async call(url: URL, input: any) : Promise<any> {
    let body = input
  ...
  async run(input: any) : Promise<string> {
    ...
    return spanManager.createActiveSpan(this.agentName,{
...
export const getAgentMetadata = async (agent: string) : Promise<Agent | null> => {
