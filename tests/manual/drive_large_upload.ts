...
const LABEL_VAL = "drive-large-upload-test"
...
// Source uses memory-backed tmpfs (/dev/shm) to host the file — no drive needed.
...
const MKFILE_TIMEOUT_MS = 10 * 60_000 // dd writing 16 GB to tmpfs (~5s) + slack
...
const DRV_PREFIXES = ["src-drive-", "tgt-drive-"]
...
async function cleanupPrevious() {
  ...
  const [sandboxes, drives] = await Promise.all([
  ...
  if (oldSbx.length + oldDrv.length === 0) {
    console.log("[pre-cleanup] nothing to clean up")
    return
  }
  console.log(`[pre-cleanup] deleting ${oldSbx.length} sandbox(es) + ${oldDrv.length} drive(s):`)
  ...
  await Promise.all(oldDrv.map(d =>
...
async function main() {
  ...
  console.log(`\n[setup] region=${REGION} fileSize=${FILE_SIZE_GB}GB sourceMem=${SOURCE_MEMORY_MB}MB`)
    ...
    // 1. Drive (target only — source uses in-memory tmpfs)
    console.log(`[setup] creating target drive: ${targetDriveName}`)
    await DriveInstance.create({ name: targetDriveName, size: TARGET_DRIVE_SIZE_GB, region: REGION, labels: LABELS })
    createdDrives.push(targetDriveName)

    // 2. Sandboxes
    console.log(`[setup] creating sandboxes: source=${sourceName} (${SOURCE_IMAGE}, ${SOURCE_MEMORY_MB}MB) receiver=${receiverName} (${RECEIVER_IMAGE}, ${RECEIVER_MEMORY_MB}MB)`)
    const [sourceSbx, receiverSbx] = await Promise.all([
      ...
      SandboxInstance.create({ name: receiverName, image: RECEIVER_IMAGE, memory: RECEIVER_MEMORY_MB, region: REGION, labels: LABELS }),
    ])
    ...
    // 4. Mount target drive on receiver at /tmp
    const receiverMountPath = "/tmp"
    console.log(`\n[mount] receiver: ${targetDriveName} → ${receiverMountPath}`)
    const mountResult = await withTimeout(
      receiverSbx.drives.mount({ driveName: targetDriveName, mountPath: receiverMountPath }),
      MOUNT_TIMEOUT_MS, "receiver drive mount"
    )
    console.log(`[mount] mount API returned:`, mountResult)

    // Verify mount actually took effect on the receiver
    const mountCheck = await receiverSbx.process.exec({
      command: `mount | grep -E '${receiverMountPath}|blfs' || echo NO_DRIVE_MOUNT_FOUND; echo '---'; df -h ${receiverMountPath} 2>&1 || true; echo '---'; ls -la ${receiverMountPath}`,
      waitForCompletion: true,
    })
    console.log(`[mount] verification on receiver:\n${mountCheck.logs ?? "(no output)"}`)
    if (mountCheck.logs?.includes("NO_DRIVE_MOUNT_FOUND")) {
      throw new Error(`drive mount did not show up in /proc/mounts at ${receiverMountPath}`)
    }
    const mountedDrives = await receiverSbx.drives.list()
    console.log(`[mount] SDK list reports:`, mountedDrives)
    ...
    // openAsBlob() returns size 0 on sparse files (blocks=0), so we write actual
    // zeros to tmpfs. With 64 GB RAM and 31 GB available on /dev/shm this fits.
    ...
    // Sanity check: tmpfs should now report the file taking real space
    ...
    // 6. Install @blaxel/core inside source sandbox + write upload script
    ...
    while (Date.now() - pollStart < UPLOAD_TIMEOUT_MS) {
      ...
      const dfReceiver = await receiverSbx.process.exec({
        command: `df -h ${receiverMountPath} 2>/dev/null | tail -1 | awk '{print $3"/"$2}'`,
        waitForCompletion: true,
      }).catch(() => null)
