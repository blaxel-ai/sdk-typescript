...
/**
 * Health check
 * Returns health status and system information including restart count and binary details
 */
 ...
 * Triggers a restart of the sandbox-api process. Returns 200 immediately before restarting.
 ...
 */
export const postRestart = <ThrowOnError extends boolean = false>(options?: Options<PostRestartData, ThrowOnError>) => {
    return (options?.client ?? _heyApiClient).post<PostRestartResponse, PostRestartError, ThrowOnError>({
        security: [
            {
                scheme: 'bearer',
                type: 'http'
